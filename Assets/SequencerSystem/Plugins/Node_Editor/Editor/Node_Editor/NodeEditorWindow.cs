...
namespace NodeEditorFramework
{
	public class NodeEditorWindow : EditorWindow 
	{
		...
		// Toolbar
		public static int toolbarHeight = 400;
		public Rect toolbarRect { get { return new Rect (0, 0, position.width, toolbarHeight); } }
		public Rect canvasWindowRect { get { return new Rect (0, toolbarHeight, position.width, position.height - toolbarHeight); } }


		#region General 

		[MenuItem ("Headache/Headache Sequencer Editor")]
		public static void CreateEditor () 
		{
			...
			_editor.titleContent = new GUIContent ("HA Sequencer", iconTexture);
		}
...
//				mainEditorState.canvasRect = canvasRect;
			...
			*/
			...
			// Draw Toolbar
			//toolbarHeight = Math.Min (600, Math.Max (200, (int)(position.height / 5)));
			toolbarHeight = (int)EditorStyles.toolbar.fixedHeight;
			NodeEditorGUI.StartNodeGUI ();
		...
		private void DrawToolbar()
		{
			...
			if (GUILayout.Button (new GUIContent ("New", "Loads an empty Sequencer"),EditorStyles.toolbarButton))
				NewNodeCanvas ();

			if (GUILayout.Button (new GUIContent ("Load", "Loads the Sequencer from a Sequencer Save File in the Assets Folder"),EditorStyles.toolbarButton)) 
			{
				string path = EditorUtility.OpenFilePanel ("Load Sequencer", NodeEditor.editorPath + "Resources/Saves/", "asset");
			...
			if (GUILayout.Button (new GUIContent ("Save", "Saves the Sequencer to a Sequencer Save File in the Assets Folder"),EditorStyles.toolbarButton))
			{
				string path = EditorUtility.SaveFilePanelInProject ("Save Sequencer", "Node Sequencer", "asset", "", NodeEditor.editorPath + "Resources/Saves/");
				if (!string.IsNullOrEmpty (path))
					SaveNodeCanvas (path);
			}
		...
		{
			GUILayout.Label (new GUIContent ("Headache Sequencer (" + mainNodeCanvas.name + ")", "Opened Canvas path: " + openedCanvasPath), NodeEditorGUI.nodeLabelBold);
